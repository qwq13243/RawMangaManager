...
class SettingsDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.cb_proxy = QCheckBox("启用代理")
        self.cb_proxy.setChecked(db.get_setting("proxy_enable") == "1")
        self.proxy_addr = QLineEdit(db.get_setting("proxy_addr"))
        form_general.addRow(self.cb_proxy, self.proxy_addr)
        
        self.cb_headless = QCheckBox("无头模式 (后台运行，不显示浏览器)")
        self.cb_headless.setChecked(db.get_setting("headless") == "1")
        form_general.addRow(self.cb_headless, QWidget())
    ...
    def save_settings(self):
        ...
        db.set_setting("proxy_enable", "1" if self.cb_proxy.isChecked() else "0")
        db.set_setting("proxy_addr", self.proxy_addr.text())
        db.set_setting("headless", "1" if self.cb_headless.isChecked() else "0")
